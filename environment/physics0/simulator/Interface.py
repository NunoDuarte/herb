...
class Interface:
    ...
    def __init__(self, bin = [10, 10, 5],
                #  foldername = '../dataset/datas/128',
                 foldername = 'dataset/datas/128/shape_vhacd',
                 visual = False,
                 scale = [1.0,1.0,1.0],
    ...
    def addBox(self, bin, scale, shift):
        box = self.makeBox(bin, color = [0.6, 0.3, 0.1, 1], thick=0.01)
    ...
    ### ADJUSTED FUNCTION TO WORK WITH ORIGIN OF THE MESH AND XYZ EULER ROTATION #######
    def addObjectC(self, name,
                  targetC = [0.0, 0.0, 0.0],
                  rotation = [0.0, 0.0, 0.0],
                  scale = None,
                  density = 1.0,
        ...
        targetC = np.array(targetC) * scale
        if name in self.shapeMap:
            mesh, visual_shape_id, collision_shape_id = self.shapeMap[name]
        else:
        ...
        assert len(rotation) == 3 or len(rotation) == 4
        if len(rotation) == 3:
            # use scipy to convert euler to quaternion
        ...
        if self.visual:
            ...
            id = p.createMultiBody(baseMass=mass,
                                   ...
                                   useMaximalCoordinates=True)
        self.meshDict[id] = mesh
        self.reset_Wraped_Position_And_Orientation_C(id, targetC)
    ...
    # check if the object is inside the box, if checkZ is not None, check if the object is inside the box and below checkZ
    def check_obj_inside(self, id, checkZ = None):
        eps = 0.03
        minC, maxC = self.get_wraped_AABB(id)
        if minC[0] <= 0 - eps or maxC[0] - self.bin[0] >= 0 + eps or \
           minC[1] <= 0 - eps or maxC[1] - self.bin[1] >= 0 + eps or \
           minC[2] <= 0 - eps:
            # print('obj out of bounds', minC, [maxC[0] - self.bin[0], maxC[1] - self.bin[1], maxC[2]])
            # key = input('Continue?')
            return False
        if checkZ is not None and maxC[2] - self.bin[2] >= checkZ:
            return False
        return True
    ...
    def reset_Wraped_Position_And_Orientation_C(self, id, targetC, targetOrientation = None):
        self.reset_trimesh_Position_And_Orientation_C(id, targetC, targetOrientation)
    ...
    def reset_trimesh_Position_And_Orientation_C(self, id, targetC, targetOrientation = None):
        if targetOrientation is not None:
            p.resetBasePositionAndOrientation(id, [-100,-100,-100], targetOrientation)
        positionFLB, orientationT, positionBase = self.get_trimesh_Position_And_Orientation(id, inner=True, getPosBase=True)
        positionTarget = targetC 
        p.resetBasePositionAndOrientation(id, positionTarget, orientationT)
    ...
    # a method that takes image from the DebugVisualizer and saves it
